...
2.  **Hyper-priors** are the distributions of the hyper-parameters. For example, $\beta_1 \sim N(\beta_0, \sigma^2_{\beta_0})$ is the hyper-prior of $\beta_1$

Basically, we have a set of layers:

Layer 1: $$ y_i|\mu_i, \beta_1 \sim P(y_i|\mu_i, \beta_0)$$

Layer 2: $$ \mu_i| \beta_1 \sim P(\mu_i|\beta_1)$$

Layer 3: $$ \beta_1 \sim P(\beta_1)$$
...
yield_lme <- data %>% 
  ...
  mutate(model = map(data,
                     ~nlme::lme(GY_bu ~ 0 + as.factor(TREAT), 
                          random = ~1|BLOCK, data=.)))
